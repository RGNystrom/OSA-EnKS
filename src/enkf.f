...
!
!--------------------------------------------------------------------------------------------------------------
subroutine enkf (wrf_file,ix,jx,kx,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,x2m,x2,ind,proj,times)
use constants
use namelist_define
use obs_define
use mpi_module
...
integer   :: ngx, ngz
integer   :: i, j, k, m, n, v, iob, iiob, nob, ie, iunit,ounit,ii, jj, kk, is, it, ig, iv, i1,j1, itot
...
real, dimension(ni,nj,nk,nv,nm), intent(inout) :: x2
real, dimension(ni,nj,nk,nv), intent(inout)    :: xm 
real, dimension(ni,nj,nk,nv), intent(inout)    :: x2m 
   ...
   do i=i1,i1+2
     if(i>=istart.and.i<=iend.and.j>=jstart.and.j<=jend) then
        if(use_simulated) then
          ...
          tmpsend(i-i1+1,j-j1+1,:,:)=x2m(i-istart+1,j-jstart+1,:,:)
        endif
     endif
   enddo
   enddo
   ...
   do n = 1, nm
     ...
     if (ie<=numbers_en+1) then
       ...
       if ( obstype(1:5) == 'Radar' ) then
         ...
         !if ( obstype == 'min_slp   ' )  write(*,'(a,i6,a,f10.2,a,i6,a,i6)')'Ens No. ', ie, ' min_slp ', center_xb(3),' i ',int(center_xb(1)),' j ',int(center_xb(2)) 
       else if ( obstype(1:1) == 'P' .or. obstype(1:1) == 'H'  ) then
...
!     write(*,'(a,i2)') 'radiance calculation called in proc ', my_proc_id
  yasend_tb=0.
  do ie = 1, numbers_en
    yasend_tb = 0.0
!    write(*,'(a,i2, a, i2)') 'radiance calc: proc ', my_proc_id, 'ie ', ie
...
!    write(  *, '(a,i3,a,f)') 'Proc ', my_proc_id, ' radiance ', yasend(
!    iob_radmin+7000,ie)
    !if (ie == numbers_en+1 .and. use_elf) then
    !   call
    !   xb_to_radiance(filename,proj,ix,jx,kx,xlong,xlat,xland,iob_radmin,iob_radmax,yasend_tb,.false.)
...
if ( my_proc_id == 0 ) then
  write( *,*) 'Finished computing HX. Now computing <Hx> for radiance and HPI.'
  write(*,'(a,f10.2)')'Ens mean min_slp', ya( 1, numbers_en+1) 
endif

! Compute linear mean of simulated observations
! Man-Yau Chan, Dec 29th 2019
! Robert Nystrom, Sep 28 2020
! Blanking out the means to be safe
if(raw%radiance%num.ne.0) then
  ya( iob_radmin:iob_radmax, numbers_en+1) = 0.0
endif
ya( 1, numbers_en+1) = 0.0 !min_slp is first observation 
! Compute linear mean
do ie=1, numbers_en
  if(raw%radiance%num.ne.0) then
    ya( iob_radmin:iob_radmax, numbers_en+1) = ya( iob_radmin:iob_radmax,numbers_en+1) + ya( iob_radmin:iob_radmax , ie) / float(  numbers_en )
  endif
  ya( 1, numbers_en+1) = ya( 1,numbers_en+1) + ya( 1, ie) / float(  numbers_en )
enddo


if ( my_proc_id == 0 ) then
  write( *,*) 'Finished computing <Hx> for radiance and min_slp.'
  write(*,'(a,f10.2)')'mean Ens min_slp', ya( 1, numbers_en+1) 
endif
...
!    2-D variables are treated as at surface k=1
   ...
   do i = uist,uied
     ...
     if (  obstype=='Radiance  ' .and. kst==1 .and. ked==1 ) then
...
!     if(varname(1:1).eq.'Q') then
     if(varname.eq.'QVAPOR    ') then
       do n=1,nm
         ie=(n-1)*nmcpu+gid+1
         if(ie<=numbers_en) then
           where( (xm(max(ist,istart)-istart+1:min(ied,iend)-istart+1, &
                      max(jst,jstart)-jstart+1:min(jed,jend)-jstart+1,kst:ked,m)+ &
                   x (max(ist,istart)-istart+1:min(ied,iend)-istart+1, &
                      max(jst,jstart)-jstart+1:min(jed,jend)-jstart+1,kst:ked,m,n))<0. ) &
                   x (max(ist,istart)-istart+1:min(ied,iend)-istart+1, &
                      max(jst,jstart)-jstart+1:min(jed,jend)-jstart+1,kst:ked,m,n)= &
                   0.-xm(max(ist,istart)-istart+1:min(ied,iend)-istart+1, &
                      max(jst,jstart)-jstart+1:min(jed,jend)-jstart+1,kst:ked,m)
         endif
       enddo
       where( xm(max(ist,istart)-istart+1:min(ied,iend)-istart+1, &
                 max(jst,jstart)-jstart+1:min(jed,jend)-jstart+1,kst:ked,m)<0.) &
              xm(max(ist,istart)-istart+1:min(ied,iend)-istart+1, &
                 max(jst,jstart)-jstart+1:min(jed,jend)-jstart+1,kst:ked,m)=0.
     endif 
     deallocate(km1)
...
if ( use_vortex_RTPP ) then !by Robert Nystrom 2018.4.7
    ...
    do n = 1, nm
      do v = 1,nv
        ie = (n-1)*nmcpu+gid+1
        if ( ie<=numbers_en+1 ) then
            if (enkfvar(v)=='SFC_ALPHA ') then
              if ( my_proc_id==0 ) write(*,*)'Var RTPP:',enkfvar(v)
              if ( my_proc_id==0 ) write(*,*)'Mixing:',mixing_alpha
              x(:,:,:,v,n) = (1.0-mixing_alpha)*x(:,:,:,v,n) + mixing_alpha*xf(:,:,:,v,n) + xm(:,:,:,v)
            elseif (enkfvar(v)=='SFC_BETA  ') then
              if ( my_proc_id==0 ) write(*,*)'Var RTPP:',enkfvar(v)
              if ( my_proc_id==0 ) write(*,*)'Mixing:',mixing_beta
              x(:,:,:,v,n) = (1.0-mixing_beta)*x(:,:,:,v,n) + mixing_beta*xf(:,:,:,v,n) + xm(:,:,:,v)
            elseif (enkfvar(v)=='SFC_VC    ') then
              if ( my_proc_id==0 ) write(*,*)'Var RTPP:',enkfvar(v)
              if ( my_proc_id==0 ) write(*,*)'Mixing:',mixing_vc
              x(:,:,:,v,n) = (1.0-mixing_vc)*x(:,:,:,v,n) + mixing_vc*xf(:,:,:,v,n) + xm(:,:,:,v)
            else
              if ( my_proc_id==0 ) write(*,*)'Var RTPP:',enkfvar(v)
              if ( my_proc_id==0 ) write(*,*)'Mixing:',mixing_core
              !call wrf_var_dimension ( wrf_file, enkfvar(v), ix, jx, kx, ii, jj, kk )
              do j = jstart,jend
                do i = istart,iend
                    ...
                    x(i-istart+1,j-jstart+1,:,v,n) = (1.0-a)*x(i-istart+1,j-jstart+1,:,v,n) + a*xf(i-istart+1,j-jstart+1,:,v,n) + xm(i-istart+1,j-jstart+1,:,v)
                end do
              end do
            endif
        endif
