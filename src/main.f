...
!   Enks algorithm by Robert Nystrom 01/2021
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  use wrf_tools
  use radar
  integer            :: i_unit=80010, o_unit=90010, u_unit=50010
  integer            :: unit
...
! ( reference to MM5_to_GrADS: module_wrf_to_grads_util.f)
  write(wrf_file,'(a5,i5.5)')'fort.',u_unit+1
...
! Initilize and Read in namelist information
  ...
  write(wrf_file,'(a5,i5.5)')'fort.',u_unit+numbers_en+1
...
allocate(x2(ni,nj,nk,nv,nm)) !State at obs time
allocate(xm(ni,nj,nk,nv))
allocate(x2m(ni,nj,nk,nv)) !mean at obs time
x=0.
xm=0.
x2=0.
x2m=0.
...
! Read in initial Ensemble in x2
if ( my_proc_id == 0 ) write(*,*)'Read in initial Ensemble in x2'
call read_ensemble(u_unit,ix,jx,kx,ni,nj,nk,nv,nm,gid,sid,iid,jid,x2)
...
call MPI_Allreduce(sum(x2,5),x2m,ntot,MPI_REAL,MPI_SUM,g_comm,ierr)
x2m=x2m/real(numbers_en)
do n=1,nm
  ie=(n-1)*nmcpu+gid+1
  if(ie==numbers_en+1) x(:,:,:,:,n)=xm
  if(ie==numbers_en+1) x2(:,:,:,:,n)=x2m
enddo
if(gid==0) &
  call output(i_unit+numbers_en+1,ix,jx,kx,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,xm,times)
if(gid==0) &
  call output(u_unit+numbers_en+1,ix,jx,kx,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,x2m,times)
...
!!! BT-based inflation if use_inf_bt by Minamide 2017.9.21
if (use_inf_bt .and. (raw%radiance%num.ne.0)) then
  if ( my_proc_id == 0 ) write(*,*)'Running BT-based inflation'
  !write(wrf_file,'(a5,i5.5)')'fort.',i_unit+1
  write(wrf_file,'(a5,i5.5)')'fort.',u_unit+1
  !call cal_inflate_bt(wrf_file,ix,jx,kx,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,ind,proj,times)
  !Old call for EnKF
  !call cal_abei(wrf_file,ix,jx,kx,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,ind,proj,times)
  !New call for EnKS
  call cal_abei(wrf_file,ix,jx,kx,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,x2m,x2,ind,proj,times)

  if ( my_proc_id == 0 ) write(*,*)'Output members and mean now...'
  do n =1, nm
    ie = (n-1)*nmcpu+gid+1
    if ( ie <= numbers_en ) then
      !call output(i_unit+ie,ix,jx,kx,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,x(:,:,:,:,n),times)
      call output(u_unit+ie,ix,jx,kx,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,x2(:,:,:,:,n),times)
    endif
  enddo

  ! re-read the inflated initial ensembles
  !x=0.
  !if ( my_proc_id == 0 ) write(*,*)'Read in inflated initial Ensemble in x'
  !call read_ensemble(i_unit,ix,jx,kx,ni,nj,nk,nv,nm,gid,sid,iid,jid,x)
  x2=0.
  if ( my_proc_id == 0 ) write(*,*)'Read in inflated initial Ensemble in x2'
  call read_ensemble(u_unit,ix,jx,kx,ni,nj,nk,nv,nm,gid,sid,iid,jid,x2)
  do n=1,nm
    ie=(n-1)*nmcpu+gid+1
    !if(ie==numbers_en+1) x(:,:,:,:,n)=xm
...
write(wrf_file,'(a5,i5.5)')'fort.',u_unit+1
call enkf(wrf_file,ix,jx,kx,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,x2m,x2,ind,proj,times)
time2 = MPI_Wtime()
