...
!---------------------
  ...
  integer                                  :: iob,irad
  real                                     :: obs_ii, obs_jj, dx,dxm,dy,dym,sat_lon

  CHARACTER(*), PARAMETER :: PROGRAM_NAME   = 'crtm'
  CHARACTER(*), PARAMETER :: RESULTS_PATH = './results/'
  !CHARACTER(*), PARAMETER :: CRTM_code ='/glade/work/nystrom/Code/CRTM_v2.3/crtm_wrf'
...
!  INTEGER, PARAMETER :: N_CLOUDS    = kx*5
  ...
  Error_Status = CRTM_Init( (/Sensor_Id/), &  ! Input... must be an array, hencethe (/../)
                            ...
                            File_Path='coefficients/', Quiet=.true.)
  ...
  elseif (Sensor_Id == 'imgr_g13' ) then
    ...
    Error_Status = CRTM_ChannelInfo_Subset( ChannelInfo(1), Channel_Subset =(/2,3,4/) )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error initializing CRTM'
      CALL Display_Message( PROGRAM_NAME, Message, FAILURE )
      STOP
