...
inflate_update = inflate
open (10, file = 'parameters_update', status = 'old', form = 'formatted', iostat = iost )
if( iost .eq. 0 ) then
  read(10,fmt='(f12.4,f12.4,f12.4)')inflate_update, error_update, infvar_update
  !inflate_update = sqrt(inflate_update)
  if (inflate_update .lt. 1.) inflate_update = 1.
  if (inflate_update .gt. inflate_max) inflate_update = inflate_max
  update_file = 1
  if ( my_proc_id == 0 ) write(*,*) 'inflate updated to ',inflate_update
else
  update_file = 0
endif
...
do n = 1, nm
   ie = (n-1)*nmcpu+gid+1
   if ( ie<=numbers_en+1 ) x(:,:,:,:,n)=inflate_update*(x(:,:,:,:,n)-xm) + xm
enddo

! Save the inflated prior
...
! Give a finish flag
finish_file_flg = times(1:4)//times(6:7)//times(9:10)//times(12:13)//times(15:16)//'.inffin_flag'
do k = 1, 12
